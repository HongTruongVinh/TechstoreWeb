...
export class LinkSettingsService {
  ...
  public GetResLinkSettingWithQueryObject(pGroup: string, pFunction: string, query?: Record<string, any>): Observable<string> {
         return this.loadResLinkSetting().pipe(
      map((resLinkSetting) => {
        const linkTemplate = resLinkSetting[pGroup]?.[pFunction];
        if (!linkTemplate) {
        ...
        const url = resLinkSetting[pGroup][pFunction];
